...
app.post('/', async (request, reply) => {
  ...
  switch (method) {
    ...
    case 'sign':
      ...
      const rPadded = Buffer.concat([Buffer.alloc(32 - signature.r.length, 0), signature.r]);
      const sPadded = Buffer.concat([Buffer.alloc(32 - signature.s.length, 0), signature.s]);
